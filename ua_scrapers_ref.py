...
ALL_CATS = [(b,e,s) for b in BASES_W_FLEETS for e in BASES_W_FLEETS[b] for s in SEATS]
...
FULL_PI_URL_PAYLOAD = {
    ...
    'ctl01$mHolder$ddlActSched' : 'S', #S = as scheduled which includes pay info, #A = actual
...
def td_hhmm(td):
    """ Helper function, takes timedelta returns string hh:mm
    ...
    """
    return f'{((td.days*24)+(td.seconds//3600)):02}:{((td.seconds//60)%60):02}'

def validate_base(b):
    """Checks to make sure given Base exists

    Args:
        b (String): Base e.g. LAX

    Raises:
        ValueError: Base does not exist
    """
    if (BASES_W_FLEETS[b] is None):
        # Base is not valid
        msg = f'The base ({text}) does not exist. Check format.'
        raise ValueError(msg)

def validate_equipment(b, e):
    """Checks if Equipment is valid for the given Base (assumed to be valid)

    Args:
        b (String): Base e.g. LAX
        e (String): Equipment e.g. 756

    Raises:
        ValueError: Equipment does not exist in Base
    """
    if (e not in BASES_W_FLEETS[b]):
        msg = f'The equipment ({e}) does not exist in ({b}).'
        raise ValueError(msg)

def validate_seat(s):
    """Checks that seat is either CA or FO

    Args:
        s (String): Seat e.g. CA

    Raises:
        ValueError: Seat does not exist
    """
    if (s not in SEATS):
        msg = f'The seat ({s}) does not exist.'
        raise ValueError(msg)

def validate_bid_month(m):
    """Checks that the given bid month is valid

    Args:
        m (String): Bid month in MMMYYYY format

    Raises:
        ValueError: Bid month does not exist
    """
    if (BID_MONTHS[m] is None):
        msg = f'The bid month ({m}) does not exist. Check MMMYYYY format.'
        raise ValueError(msg)        

# Validate date.. sorry Nick. The joke was good while it lasted.
def validate_date(date):
    """Checks that provided date is valid

    Args:
        date (String): Date in DDMMYY format

    Raises:
        ValueError: Date not valid
    """
    try:
        d,m,y = int(date[0:2]), int(date[2:4]), int(date[4:6]) + 2000
        datetime.date(y,m,d)
    except:
        msg = f'The date in DDMMYY format you provided ({date}) is not valid.'
        raise ValueError(msg)
